...
export class AutenticadorController {
  ...
  async registrarUsuario(@Body() autenticadorDto: AutenticadorDto) {
    try {
      ...
      const respuesta =
        await this.autenticadorService.registrarUsuario(autenticadorDto);
      console.log('Usuario registrado exitosamente:', respuesta);
      return {
        ok: true,
        mensaje:
          'Usuario registrado exitosamente. Revisa tu correo electrónico.',
        respuesta,
      };
    } catch (error) {
  ...
  async iniciarSesion(@Body() autenticadorDto: AutenticadorDto) {
    const resultado =
      await this.autenticadorService.iniciarSesion(autenticadorDto);
  ...
  async olvidoContraseña(@Body() body: { email: string }) {
    try {
      const resultado = await this.autenticadorService.olvidoContraseña(
        body.email,
      );
      if (resultado) {
        return {
  ...
  async restablecerContraseña(
    ...
    try {
      const resultado = await this.autenticadorService.restablecerContraseña(
        body.token,
        body.newPassword,
      );
      if (resultado) {
