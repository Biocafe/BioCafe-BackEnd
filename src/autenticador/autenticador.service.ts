...
export class AutenticadorService {
  constructor(
    @InjectModel('Autenticador')
    private autenticadorModel: Model<IAutenticador>,
    private emailService: EmailService,
  ) {}

  // MÉTODO PARA REGISTRAR USUARIO
  async registrarUsuario(
    autenticadorDto: AutenticadorDto,
  ): Promise<IAutenticador> {
    ...
    try {
      await this.emailService.enviarCorreoBienvenida(
        usuario,
        usuario.split('@')[0],
      );
  ...
  async iniciarSesion(
    ...
    const usuarioEncontrado = await this.autenticadorModel
      .findOne({ usuario })
      .exec();
  ...
  async olvidoContraseña(email: string): Promise<string | null> {
    ...
    const resetToken = jwt.sign(
      { usuario: usuarioEncontrado.usuario, reset: true },
      'clave_secreta',
      { expiresIn: '1h' },
    );
